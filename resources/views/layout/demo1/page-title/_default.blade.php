...
<div {{ theme()->printHtmlAttributes("page-title") }} class="d-flex {{ $baseClass }}">
    ...
    <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">
        IETF Meeting {{ App\Models\MeetingInfo::where('active', 1)->pluck('meetingNumber')->first() }} |
            @if(Request::is('sessions/*') && Request::route('id'))
                @php
                    $id = Request::route('id');
                    $pt = App\Models\Session::where('id', $id)->pluck('parent')->first();
                    echo 'Session: '.strtoupper($pt);
                @endphp
            @elseif(Request::is('participants/*') && Request::route('id'))
                @php
                    $id = Request::route('id');
                    //echo Request::url();
                    $p = App\Models\Participant::where('id', $id)->pluck('username')->first();
                    echo 'Participant: '.strtoupper($p);
                @endphp
            @else
